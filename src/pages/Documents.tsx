...
import { Upload, FileText, Home, DollarSign, Calendar, User, CreditCard, Landmark, Loader2 } from "lucide-react";
...
const Documents = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requiredDocs: (keyof DocumentFiles)[] = ['rgFront', 'rgBack', 'selfie', 'selfieWithRg', 'proofOfResidence', 'proofOfIncome'];
    ...
    try {
      const fileUploads = [
        ...
        uploadFile(documents.selfie!, 'selfie'),
      ...
      const [rgFrontUrl, rgBackUrl, selfieUrl, selfieWithRgUrl, proofOfResidenceUrl, proofOfIncomeUrl] = urls;
              ...
              <CardContent className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="rgFront" name="rgFront" type="file" accept="image/*" onChange={handleFileChange} className="cursor-pointer" required />
                    {documents.rgFront && <p className="text-xs text-primary mt-1">✓ {documents.rgFront.name}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="rgBack">RG - Verso *</Label>
                    <Input id="rgBack" name="rgBack" type="file" accept="image/*" onChange={handleFileChange} className="cursor-pointer" required />
                    {documents.rgBack && <p className="text-xs text-primary mt-1">✓ {documents.rgBack.name}</p>}
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="selfie">Selfie *</Label>
                  <Input id="selfie" name="selfie" type="file" accept="image/*" onChange={handleFileChange} className="cursor-pointer" required />
                  {documents.selfie && <p className="text-xs text-primary mt-1">✓ {documents.selfie.name}</p>}
                </div>
                <div className="space-y-2">
                  <Label htmlFor="selfieWithRg">Selfie segurando o RG *</Label>
                  <Input id="selfieWithRg" name="selfieWithRg" type="file" accept="image/*" onChange={handleFileChange} className="cursor-pointer" required />
            ...
            <Card className="border-2 border-primary/20 shadow-elegant animate-slide-up">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <Input id="proofOfResidence" name="proofOfResidence" type="file" accept="image/*,.pdf" onChange={handleFileChange} className="cursor-pointer" required />
            ...
            <Card className="border-2 border-primary/20 shadow-elegant animate-slide-up">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <Input id="proofOfIncome" name="proofOfIncome" type="file" accept="image/*,.pdf" onChange={handleFileChange} className="cursor-pointer" required />
